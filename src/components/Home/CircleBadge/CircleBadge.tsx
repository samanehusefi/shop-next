  ...
  return (
    <>
      <section className="mx-auto w-full max-w-350 px-4 py-2">
        <div className="rounded-2xl bg-white py-2 md:px-4">
          <div className="flex w-full gap-4 overflow-x-auto px-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:justify-between md:gap-4">
            {visibleItems.map((item) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex w-[calc((100vw-6.5rem)/5.5)] min-w-[calc((100vw-6.5rem)/5.5)] shrink-0 flex-col items-center text-center sm:w-[100px] sm:min-w-[100px]"
              >
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="absolute inset-0 w-full overflow-y-auto bg-white p-5 animate-[slideUp_1.2s_ease-in-out] md:inset-auto md:left-1/2 md:top-1/2 md:max-h-[85vh] md:w-[90%] md:max-w-4xl md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-2xl md:p-6 md:animate-none"
          ...
          >
            ...
            <button
              type="button"
              onClick={() => setIsModalOpen(false)}
              className="absolute left-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200"
            >
