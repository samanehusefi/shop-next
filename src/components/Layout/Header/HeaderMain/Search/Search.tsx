...
const Search = () => {
  ...
  return (
    <div
      ref={searchRef}
      className="relative w-full flex-1 md:max-w-xl lg:max-w-2xl"
    >
      <div
        className="flex h-10 w-full items-center gap-2 rounded-full bg-gray-100 px-3 sm:h-11 sm:gap-3 sm:px-4 md:h-12"
        dir="rtl"
      >
        ...
        {searchText && (
          <button
            type="button"
            onClick={handleClear}
            className="flex h-6 w-6 shrink-0 cursor-pointer items-center justify-center rounded-full text-base leading-none text-gray-400 transition hover:bg-gray-200 hover:text-gray-700 sm:h-7 sm:w-7"
      ...
      {showDropdown && (
        <div
          dir="rtl"
          className="absolute top-[46px] right-0 left-0 z-[300] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg sm:top-[50px] md:top-[54px]"
        >
          {results.length > 0 ? (
            <div className="max-h-[60vh] overflow-y-auto py-1 sm:py-2">
              {categories.length > 0 && (
                <div>
                  <div className="px-3 py-2 text-[11px] font-bold text-gray-500 sm:px-4 sm:text-xs">
                  ...
                  {categories.map((result, index) => (
                    <div
                      key={`category-${index}`}
                      className="cursor-pointer px-3 py-2.5 text-xs text-gray-700 transition hover:bg-gray-50 sm:px-4 sm:py-3 sm:text-sm"
                    >
                      {result.title}
                    </div>
                  ))}
              ...
              {products.length > 0 && (
                <div className="border-t border-gray-100">
                  ...
                  {products.map((result, index) => (
                    <div
                      key={`product-${index}`}
                      className="cursor-pointer truncate px-3 py-2.5 text-xs text-gray-700 transition hover:bg-gray-50 sm:px-4 sm:py-3 sm:text-sm"
                    >
                      {result.title}
                    </div>
                  ))}
            ...
            <div className="flex min-h-[260px] flex-col items-center justify-center gap-2 p-4 text-center sm:min-h-[300px] sm:p-6">
              <img
